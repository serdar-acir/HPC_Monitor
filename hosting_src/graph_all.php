...
if ($debug == 1) {
    $_GET['cl'] = "SakuraHPC";
    $_GET['cn'] = "cn01";
    $_GET['param'] = "cpu_usage";
    $_GET['ref_time'] = "2023-02-20 18:49:14";
}
...
$param = $_GET['param'] ?? null; if ($param == NULL) $param = $_POST['param'] ?? null;
$ref_time = $_GET['ref_time'] ?? null; if ($ref_time == NULL) $ref_time = $_POST['ref_time'] ?? null;
...
for ($i = 1; $i <= $nrows; $i++) { //for1 start
    ...
    if ($i == 1) $baslangic_time = $report_time[$i];
    else if ($i == $nrows) $bitis_time = $report_time[$i];

    $prev_report_time = $report_time[$i - 1] ?? null;

    if ($report_time[$i] != $prev_report_time) { //degisti start
        if ($subset_counter > 0) {
            $usage[$m] = round($ara_toplam / $subset_counter);
            $m++;
        }

        if ($row19[$param] == NULL) $row19[$param] = 0;
        if ($row19["node_stat"] != "idle") $row19[$param] = 100;

        $ara_toplam = $row19[$param];
        $subset_counter = 1;
    } //degisti end
        ...
        $ara_toplam = $ara_toplam + $row19[$param];
        $subset_counter++;
    } //degismedi end
...
if ($param == "cpu_usage") {
    $title = "CPU Utilization for $cluster_name for all nodes";
    $ort_ek = "Start: $baslangic_time\nEnd: $bitis_time";
    $aYMin = 0;
    $aYMax = 100;
    $aXMin = 0;
    $aXMax = $m;
} else {
    $title = "$param for $cluster_name for all nodes";
    $ort_ek = "Start: $baslangic_time\nEnd: $bitis_time";
    $aYMin = 0;
    $aYMax = 100;
    $aXMin = 0;
