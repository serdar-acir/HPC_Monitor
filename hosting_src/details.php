...
<?php
$cluster_name = $_GET['cl'] ?? null; if ($cluster_name == NULL) $cluster_name = $_POST['cl'] ?? null;
$node_name = $_GET['cn'] ?? null; if ($node_name == NULL) $node_name = $_POST['cn'] ?? null;
$ref_time_w = $_POST['ref_time_w'] ?? null; if ($ref_time_w == NULL) $ref_time_w = $_GET['ref_time_w'] ?? null;
$iparam = $_POST['iparam'] ?? null; if ($iparam == NULL) $iparam = $_GET['iparam'] ?? null;
...
$adm_link = "<span class=\"admin-link\"><a href=\"admin.php?$mylink\">admin</a></span>";
...
echo "<div class=\"topbar\">
        <div class=\"title-wrap\">
          <h3><a href=\"?$mylink&cluster_name=$cluster_name\">$cluster_name_i ::: ".strtoupper($node_name)." - Benchmark Details</a></h3>
          <div class=\"subtitle\">$adm_link</div>
        </div>
      </div>";
...
echo "<div class=\"controls\">
        <table>
          <tr>
            <td>
              <form method=\"post\">
                ...
                <input type=\"hidden\" id=\"cl\" name=\"cl\" value=\"$cluster_name\">
                <input type=\"hidden\" id=\"cn\" name=\"cn\" value=\"$node_name\">
              </form>
            </td>
            <td>
              <form method=\"post\">
          ...
          <tr>
            <td colspan=\"2\">
              <button class=\"collapsible\">".strtoupper($cluster_name_i)." - ".strtoupper($node_name)." Hardware Information</button>
              <div class=\"content\">$that_hw</div>
            </td>
          </tr>
        </table>
      </div>";
...
if ($iparam == "all" OR $iparam == "cpu_usage") {
    echo "<tr><td width=10%>CPU Utilization</td><td colspan=3><img src=\"graph.php?cl=$cluster_name&cn=$node_name&ref_time=$ref_time&param=cpu_usage\" width=\"400\" height=\"200\" class=\"img\" /></td><tr>";
}
if ($iparam == "all" OR $iparam == "ram_usage") {
    echo "<tr><td width=10%>RAM Utilization</td><td><img src=\"graph.php?cl=$cluster_name&cn=$node_name&ref_time=$ref_time&param=ram_usage\" width=\"400\" height=\"200\" class=\"img\" /></td>
...
if ($iparam == "all" OR $iparam == "disk_write_MBs") {
    echo "<tr><td width=10%>Disk Write Speed</td><td colspan=3><img src=\"graph.php?cl=$cluster_name&cn=$node_name&ref_time=$ref_time&param=disk_write_MBs\" width=\"400\" height=\"200\" class=\"img\" /></td><tr>";
}
if ($iparam == "all" OR $iparam == "nw_speed_Mbs") {
    echo "<tr><td width=10%>Network Speed</td><td colspan=3><img src=\"graph.php?cl=$cluster_name&cn=$node_name&ref_time=$ref_time&param=nw_speed_Mbs\" width=\"400\" height=\"200\" class=\"img\" /></td><tr>";
}
echo "</table></div>";

if ($iparam != "all") {
    $that_data = get_all_data($cluster_name, $node_name, $ref_time, $iparam);
    echo "<div class=\"controls\"><table><tr><td><button class=\"collapsible\">".strtoupper($cluster_name_i)." - ".strtoupper($node_name)." ".strtoupper($iparam)." DATA</button><div class=\"content\">$that_data</div></td></tr></table></div>";
}
...
function get_all_hardware($cluster_name, $node_name) {
    ...
    for ($i=1; $i<=$nrows; $i++) {
        ...
        extract($row19);
        $message = $message."<br><b>$category</b>: <span style=\"color:darkgreen\">".nl2br($veri)."</span>";
    }
...
function get_all_data($cluster_name, $node_name, $ref_time, $iparam) {
    ...
    for ($i=1; $i<=$nrows; $i++) {
        $row19 = mysqli_fetch_array($result_generic2 ,MYSQLI_ASSOC);
        extract($row19);
        if ($report_time >= $ref_time) {
            if ($node_stat != "idle") {
                $message = $message."<b>$report_time</b>: <span style=\"color:darkred\">==> <b>".strtoupper($node_stat)."</b></span><br>";
            } else {
                ...
                $message = $message."<b>$report_time</b>: <span style=\"color:darkgreen\">==> ${$iparam}</span> $metric_ek";
                if ($iparam == "ram_usage") {
                    $message = $message."  <i><b>mem_available / mem_total: <span style=\"color:darkgreen\">".sprintf("%.2f", $mem_available/1000000)." / ".sprintf("%.2f", $mem_total/1000000)." GB </span><b>swap_used:</b> <span style=\"color:darkgreen\">".sprintf("%.2f", $swap_used/1000000)." GB</span></i><br>";
                } else {
                    $message = $message."<br>";
                }
            }
