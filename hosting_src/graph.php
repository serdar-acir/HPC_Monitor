...
for ($i = 1; $i <= $nrows; $i++) {
    ...
    $report_time_short[$i] = $row19['report_time'];

    $usage[$i] = $row19["$param"];
    if ($usage[$i] == NULL) $usage[$i] = 0;

    if ($param == "mem_available") {
        $usage[$i] = sprintf("%.2f", $usage[$i] / 1000000);
        $total_mem = sprintf("%.2f", $row19['mem_total'] / 1000000);
    }
    else if ($param == "swap_used") {
        $usage[$i] = sprintf("%.2f", $usage[$i] / 1000000);
    }
    else if ($param == "nw_speed_Mbs") {
        $usage[$i] = sprintf("%.2f", $usage[$i] / 1000);
    }
...
$report_time_short = array_reverse($report_time_short);
...
else { $title = $param; $ort_ek = ""; $aYMin=0; $aYMax=$usage_max; $aXMin=0; $aXMax=$i; }

if ($i > 0) $ortalama = sprintf("%.2f", $total_usage / $i);
else $ortalama = "0.00";
