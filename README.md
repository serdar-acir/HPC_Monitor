...
1. Make sure the Requirement mentioned above is installed on login and compute nodes. Clone this repository to the a user environment on the login node.
2. Setup a separate maria-db based web server on a hosting platform.
3. On the hosting server set up your maria-db database and visit to hosting_src/setup.php page via a browser. Enter the required data again and complete the installation. This will create necessary links.
4. At the login node access to the login_node_src folder and configure HPC1.config file according to your specific HPC environment needs as described in its README file.
